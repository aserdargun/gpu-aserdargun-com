...
async function expectLocalizedDocumentHead(page: Page, locale: "tr" | "en") {
  ...
  await expect(page.locator('head meta[property="og:description"]')).toHaveAttribute("content", english ? "11 atlases · 12 weeks · One learning system" : "11 atlas · 12 hafta · Tek öğrenme sistemi");
  ...
  await expect(page.locator('head meta[name="twitter:description"]')).toHaveAttribute("content", english ? "11 atlases · 12 weeks · One learning system" : "11 atlas · 12 hafta · Tek öğrenme sistemi");
    ...
    for (const moduleId of moduleIds) {
      ...
      const check = async () => {
        actualChecks += await expectEveryActualScrollerKeyboardFocus(
          page,
          root,
          viewport.width === 320 && moduleId !== "architecture",
...
for (const task6Case of [
  ...
  test(`Task 6 ${task6Case.locale} overview keeps the roadmap, maturity policy, and responsive shell connected`, async ({ page }) => {
    ...
    await expect(page.locator(".module-grid .module-card")).toHaveCount(11);
  ...
  test(`${locale} search matches localized content and visible architecture metadata`, async ({ page }) => {
    ...
    const localizedCases = locale === "tr"
      ? [
          ...
          { query: "Temel", ids: ["toolchain", "correctness"] },
        ]
      : [
          { query: "Engineering Foundations", ids: ["toolchain"] },
          { query: "bank conflicts", ids: ["memory"] },
          { query: "opcheck", ids: ["triton"] },
          { query: "Core", ids: ["toolchain", "architecture", "memory", "triton", "operators", "correctness", "profiling", "cutlass", "inference", "systems"] },
        ];
    const architectureCases = [
      { query: "Rubin", ids: ["architecture", "cutlass", "systems"] },
      { query: "core baseline", ids: ["toolchain", "architecture", "memory", "triton", "operators", "correctness", "profiling", "cutlass", "inference", "systems"] },
...
test("malformed local state recovers without blocking the overview", async ({ page }) => {
  ...
  await expect(page.getByText("0/11 ATLAS", { exact: true })).toBeVisible();
...
test("denied storage does not block startup or locale and session UI updates", async ({ page }) => {
  ...
  await gotoAtlas(page, "tr");
  await expect(page.locator("h1")).toBeVisible();
  await expect(page.getByText("0/11 ATLAS", { exact: true })).toHaveCount(1);
  await expect(page.getByTestId("atlas-continue")).toHaveCount(0);
