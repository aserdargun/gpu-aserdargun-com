...
test("Task 6 roadmap keeps the bilingual evidence cadence, eleven modules, and non-graduation Preview boundary", async () => {
  ...
  assert.equal(uiByLocale.tr.hero, "On bir etkileşimli atlas, tile düzeyi programlama, Blackwell farkındalıklı optimizasyon ve dağıtık çıkarımı; desteklediğiniz mimari ve backend sınırlarını görünür kılan 12 haftalık kanıt rotasında birleştirir.");
  assert.equal(uiByLocale.en.hero, "Eleven interactive atlases connect tile-level programming, Blackwell-aware optimization, and distributed inference in a 12-week evidence route that makes your supported architecture and backend boundaries visible.");
