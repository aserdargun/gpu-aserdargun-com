...
test("server-renders the complete English shell with request-aware production metadata", async () => {
  ...
  assert.match(html, /12 atlases · 12 weeks · One learning system/);
...
test("routes active lab content through renderLab and ModuleFrame", async () => {
  ...
  for (const [kind, component] of [
    ["visual", "VisualLab"], ["toolchain", "ToolchainLab"], ["architecture", "ArchitectureLab"], ["memory", "MemoryLab"],
  ...
  ]) assert.match(renderLab, new RegExp(`if \\(kind === "${kind}"\\) specialist = <${component} locale=\\{locale\\} />`));
  assert.match(renderLab, /<ConceptStudio kind=\{kind\} locale=\{locale\} \/>/);
});
...
test("Task 6 renders the Azure-built bilingual roadmap, maturity policy, and preserved social-card contract", async () => {
  ...
  assert.deepEqual(registry.MODULE_IDS, ["visual", "toolchain", "architecture", "memory", "triton", "operators", "correctness", "profiling", "cutlass", "inference", "multigpu", "systems"]);
  assert.match(metadata, /og\.png/);
  assert.match(metadata, /og-en\.png/);
