...
const weeks = [
  { id: 1, title: "Tensor anatomy", eyebrow: "Basis", desc: "See Stride, layout and eager execution with the naked eye.", status: "done", minutes: 90, skills: ["stride", "broadcast", "profiling"] },
  { id: 2, title: "custom operator", eyebrow: "PyTorch", desc: "Install schema, CPU reference and fake kernel with torch.library.", status: "active", minutes: 120, skills: ["torch.library", "FakeTensor", "opcheck"] },
  ...
  { id: 4, title: "Autograd + compile", eyebrow: "Entegrasyon", desc: "Save backpropagation; Hunt for graph break with torch.compile.", status: "locked", minutes: 150, skills: ["register_autograd", "torch.compile", "AOT"] },
  { id: 5, title: "RMSNorm", eyebrow: "Operator 01", desc: "From reference to fused Triton kernel, measure on three figures.", status: "locked", minutes: 180, skills: ["reduction", "numerics", "fusion"] },
  ...
  { id: 8, title: "Masked softmax", eyebrow: "Operator 04", desc: "Stable reduction, masking and limit states.", status: "locked", minutes: 210, skills: ["online softmax", "masking", "NaN"] },
  { id: 9, title: "KV-cache update", eyebrow: "Operator 05", desc: "Secure and fast writing to Paged memory layout.", status: "locked", minutes: 210, skills: ["scatter", "cache", "race"] },
  { id: 10, title: "benchmark science", eyebrow: "Performans", desc: "Honest comparison with warm-up, quantile and roofline.", status: "locked", minutes: 150, skills: ["triton.testing", "roofline", "Nsight"] },
  { id: 11, title: "fusion studio", eyebrow: "Optimizasyon", desc: "Aim for at least 15% median improvement across two fused kernels.", status: "locked", minutes: 240, skills: ["fusion", "register pressure", "autotune"] },
  { id: 12, title: "Inference capstone", eyebrow: "Mezuniyet", desc: "Generate TTFT, ITL and throughput report in vLLM workload.", status: "locked", minutes: 300, skills: ["vLLM", "TTFT", "portfolio"] },
];
...
const quizOptions = [
  "Each program processes the entire tensor; The mask only increases the speed.",
  "Programs process fixed blocks; If the last block overflows, the mask covers invalid addresses.",
    ...
    <main className="pytorch-triton-embed">
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-copy">
          ...
          <div className="hero-actions">
            ...
            <a className="text-link" href="#path">Check out the program <span>↓</span></a>
          </div>
        </div>
      ...
      <section className="section roadmap" id="path">
        <div className="section-heading">
          ...
          <p>A single mental model each week; turns working code into test evidence and performance report.</p>
        </div>
        ...
        <p className="roadmap-total"><strong>{formatTime(totalMinutes)}</strong> core laboratory · 5 mandatory LLM operators · 1 capstone</p>
      </section>
        ...
        <div className="lab-shell">
          ...
          <div className="lab-main">
            <div className="editor-pane">
              <div className="editor-heading"><span>{codeTab === "triton" ? "TRITON APPLICATION" : "PYTORCH CUSTOM OP"}</span><span>Python</span></div>
            ...
            <aside className="task-pane">
              ...
              <div className="checks">
                ...
                <label><input type="checkbox" /><span>Apply border mask</span></label>
              </div>
              <div className="hint"><span>CLUE</span><p>Each program processes one block. For valid elements <code>offsets &lt; n</code> use the expression.</p></div>
              <button className={`run-button ${runState}`} onClick={runTests} disabled={runState === "running"}>
                <span>{runState === "running" ? "TESTS ARE WORKING" : runState === "passed" ? "RUN AGAIN" : "RUN TESTS"}</span><b>{runState === "running" ? "···" : "▶"}</b>
              </button>
            </aside>
          </div>

          <div className={`console ${runState}`} aria-live="polite">
            <div className="console-title"><span>TEST CONSOLE</span><span>{runState === "passed" ? "4/4 PASSED" : runState === "running" ? "WORKING" : "HAZIR"}</span></div>
      ...
      <section className="section model-section" id="model">
        <div className="section-heading">
          <div><span className="section-number">03</span><p className="eyebrow">MENTAL MODEL</p><h2>How does a kernel<br /><em>thinker?</em></h2></div>
          <p>Play parameter; See the relationship between grid, program and memory access.</p>
        </div>
      ...
      <section className="section checkpoint">
        <div className="checkpoint-copy">
          ...
          <h2>that you understand<br /><em>prove.</em></h2>
          <p>Answer without looking at your notes. The wrong answer is the data of the learning cycle.</p>
        </div>
        <div className="quiz-card">
          ...
          {quiz !== null && <p className={`feedback ${quiz === 1 ? "success" : "retry"}`}>{quiz === 1 ? "TRUE. The mask prevents the final program from accessing unallocated memory." : "Think again: the offsets of the final program may exceed the tensor limit."}</p>}
        </div>
      </section>
      ...
      <section className="section journal">
        <div>
          ...
          <h2>what today<br /><em>did you really learn?</em></h2>
        ...
        <div className="note-area">
          ...
          <div><span>{note.length} character · is stored on this device</span><button onClick={saveNote}>{saved ? "SAVED ✓" : "SAVE NOTE"}</button></div>
        </div>
      </section>

      <footer>
        <div className="footer-brand"><span className="brand-mark">K//</span><h2>Speed you do not measure<br />is not evidence.</h2></div>
        <div className="footer-stats"><div><strong>12</strong><span>week</span></div><div><strong>18</strong><span>lab</span></div><div><strong>05</strong><span>LLM operator</span></div></div>
