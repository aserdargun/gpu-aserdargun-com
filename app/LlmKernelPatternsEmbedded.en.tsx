...
const topicCopy: Record<TopicId, { kicker: string; title: string; lead: string; formula: string }> = {
  ...
  attention: {
    ...
    lead: "Attention combines QKᵀ scores, scaling, masking, softmax, and a weighted sum over V. Flash-style kernels run online softmax on tiles without writing the full score matrix to memory.",
...
const quiz: Record<TopicId, { q: string; options: string[]; answer: number; note: string }> = {
  gemm: { q: "What is the main benefit of tiling in GEMM?", options: ["Starting more threads", "Reusing data loaded from global memory", "Removing the K dimension"], answer: 1, note: "A tile can feed many FMAs while it remains in shared memory or registers." },
  ...
  softmax: { q: "Why is max(x) subtracted before exponentiation?", options: ["To reset the total", "To change the order", "To prevent overflow"], answer: 2, note: "Subtracting the same constant from every score leaves the distribution unchanged and makes the largest exponential value 1." },
  normalization: { q: "Which LayerNorm step does RMSNorm omit?", options: ["Mean centering", "Mean-square scaling", "Learned γ"], answer: 0, note: "RMSNorm does not center the input; it computes an RMS scale and applies the learned γ." },
  ...
  return (
    <div className="lab-grid">
      <section className="panel visual-panel">
        ...
        <p className="visual-caption"><i /> Each block loads A and B tiles into fast memory; its threads update register accumulators along the K axis.</p>
      </section>
...
function KernelPattern({ topic }: { topic: TopicId }) {
  const content: Record<TopicId, { title: string; items: { n: string; h: string; p: string }[]; code: string[] }> = {
    ...
    reduction: { title: "Hierarchical reduction", items: [{ n: "01", h: "Thread-local", p: "Each thread produces a local result from strided inputs." }, { n: "02", h: "Warp combine", p: "Shuffle operations combine values held in registers." }, { n: "03", h: "Block finalize", p: "Warp-level results are combined in a small shared-memory region." }], code: ["acc = identity", "for i in thread_strided_indices:", "acc = op(acc, x[i])", "acc = warp_reduce(acc)", "if lane == 0: partial[warp] = acc"] },
    softmax: { title: "From three passes to one kernel", items: [{ n: "01", h: "Row maximum", p: "A parallel reduction finds the row maximum." }, { n: "02", h: "Exp + sum", p: "Shifted exponentials and their sum are computed in the same tile." }, { n: "03", h: "Normalize + store", p: "Register-held values are divided by the denominator and written to output." }], code: ["x = load(row, mask=cols < N)", "m = max(x, axis=0)", "z = exp(x - m)", "l = sum(z, axis=0)", "store(out, z / l)"] },
    normalization: { title: "Reduction + pointwise fusion", items: [{ n: "01", h: "Load once", p: "The activation row is loaded once with coalesced accesses." }, { n: "02", h: "Compute statistics", p: "The block computes μ/σ² or the RMS scale." }, { n: "03", h: "Affine + residual", p: "Fuse γ, β, and the residual path in the same kernel when appropriate." }], code: ["x = load(row)", "rms = sqrt(mean(x * x) + eps)", "y = x * rsqrt(rms * rms)", "y = y * gamma", "store(out, y)"] },
    attention: { title: "IO-aware attention", items: [{ n: "01", h: "Keep the Q tile", p: "Keep the Q tile in registers or shared memory." }, { n: "02", h: "Stream K/V", p: "Move K and V tiles through fast memory in sequence." }, { n: "03", h: "Online softmax", p: "Rescale the running maximum and sum as each new tile arrives." }], code: ["for kv_tile in sequence:", "scores = dot(q, k.T) * scale", "m_new = max(m, max(scores))", "l = l * exp(m-m_new) + sum(exp(scores-m_new))", "out = rescale(out) + exp(scores-m_new) @ v"] },
