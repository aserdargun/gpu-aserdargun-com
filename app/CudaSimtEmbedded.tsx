...
export const CUDA_PROGRAMMING_BRIDGE = [
  { id: "thread", title: "İş parçacığı / şerit", body: "SIMT kernelinde her şerit kendi indeksini, denetim akışını ve yazmaç durumunu taşır." },
  { id: "warp", title: "Warp iş birliği", body: "32 şerit ortak talimat akışında; etkin maske, birleşik erişim ve warp düzeyi ilkellerle iş birliği yapar." },
  { id: "tile", title: "Döşeme işlemi", body: "Programcı çok boyutlu bir veri döşemesini yükler, dönüştürür ve saklar; iş parçacığı eşlemesini tek tek yazmaz." },
  { id: "mapping", title: "Derleyici / çalışma zamanı eşlemesi", body: "CUDA Tile IR ve derleyici, döşeme işlemlerini blok içindeki iş parçacıklarına ve uygun donanım yollarına eşler." },
] as const;
...
const archData: Record<ArchLevel, { label: string; code: string; title: string; body: string; owner: string; sharing: string; result: string }> = {
  grid: {
    ...
    owner: "Ana sistem tarafındaki kernel çalıştırması", sharing: "Bloklar global bellek üzerinden haberleşebilir; normal bir kernel içinde genel ızgara bariyeri yoktur.",
    result: "Blok sırasına güvenme; bloklar herhangi bir sırada çalışabilir.",
  },
  block: {
    label: "Blok", code: "blockIdx", title: "Blok · iş birliği ve kaynak tahsisi",
    body: "Birlikte zamanlanan iş parçacığı grubudur. Bir blok ömrü boyunca tek bir SM’de kalır ve warplara bölünür.",
    owner: "Izgara zamanlayıcısı → uygun bir SM", sharing: "Aynı bloktaki iş parçacıkları paylaşımlı belleği kullanabilir ve __syncthreads() ile bariyer kurabilir.",
    result: "Yazmaç veya paylaşımlı bellek tüketimi yükselirse aynı SM’de eşzamanlı yaşayabilen blok sayısı düşebilir.",
  },
  warp: {
    label: "Warp", code: "32 iş parçacığı", title: "Warp · temel gönderim / zamanlama grubu",
    body: "32 ardışık iş parçacığından oluşur. Zamanlayıcı hazır warplardan talimat göndererek bellek ve yürütme hattı beklemelerini gizlemeye çalışır.",
    owner: "SM warp zamanlayıcısı", sharing: "Şeritler warp düzeyi ilkellerle yazmaç verisi paylaşabilir; etkin maske önemlidir.",
    result: "Aynı warp içindeki dallanma ayrışması yolları maskelerle ardışık hâle getirebilir.",
  },
  thread: {
    label: "İş parçacığı", code: "threadIdx", title: "İş parçacığı · bağımsız program durumu",
    body: "Kendi indeksi, yazmaçları ve yerel verisi vardır; aynı kernel kodunu farklı veri üzerinde çalıştırır.",
    owner: "Bir warp içindeki şerit kimliği", sharing: "Yazmaçlar özeldir; paylaşımlı bellek blok, global bellek aygıt kapsamındadır.",
    result: "İş parçacığı mantıksal bir program örneğidir; kalıcı fiziksel bir CUDA çekirdeği değildir.",
  },
  instruction: {
    label: "Talimat", code: "şerit işlemi", title: "Talimat · yürütme hattı işi",
    body: "Warp talimatı; FP/INT, yükleme-depolama, özel işlev veya tensör yürütme hatlarından uygun olana gönderilir.",
    owner: "Warp zamanlayıcısı + gönderim", sharing: "Etkin şerit maskesi hangi iş parçacıklarının sonuç yazacağını belirler.",
    result: "İş hacmi; talimat karışımı, bağımlılıklar, hazır warplar ve yürütme birimi kapasitesiyle şekillenir.",
  },
};

const memoryData: Record<MemoryLevel, { title: string; scope: string; body: string; risk: string }> = {
  register: { title: "Yazmaçlar", scope: "İş parçacığı", body: "Her iş parçacığının özel çalışma alanıdır. Çok düşük gecikme ve yüksek bant genişliği sunar; SM başına toplam bütçe sınırlıdır.", risk: "Yazmaç baskısı doluluğu azaltabilir; taşma olursa yerel bellek erişimi global bellek yoluna düşer." },
  shared: { title: "Paylaşımlı bellek / L1", scope: "Blok / SM", body: "Yonga üzeri kaynaktır. Paylaşımlı belleği blok içi yeniden kullanım için yazılımcı, L1 erişimlerini donanım yönetir.", risk: "Banka çakışması erişimi ardışık hâle getirebilir; yüksek tahsis yerleşik blok sayısını düşürebilir." },
  l2: { title: "L2 önbelleği", scope: "Tüm aygıt", body: "Tüm SM’lerce paylaşılan aygıt düzeyi önbellektir; global bellek trafiğini azaltabilir.", risk: "Çalışma kümesi büyük veya erişim düzensizse isabet oranı düşebilir." },
  global: { title: "Genel bellek", scope: "Tüm aygıt", body: "Yüksek kapasiteli GDDR/HBM'dir. Yüksek gecikmesine karşın düzenli ve paralel erişimde yüksek bant genişliği sağlar.", risk: "Birleşik erişim zayıfsa küçük miktarda yararlı veri için çok fazla sektör taşınır." },
};
...
const quizQuestions: QuizQuestion[] = [
  { q: "Bir warp kaç iş parçacığından oluşur?", options: ["8", "16", "32", "64"], answer: 2, explain: "Warp, 32 ardışık iş parçacığından oluşan temel gönderim ve zamanlama grubudur." },
  ...
  { q: "Global iş parçacığı indeksi nasıl hesaplanır?", options: ["i = threadIdx.x", "i = blockIdx.x", "i = blockIdx.x * blockDim.x + threadIdx.x", "i = blockIdx.x + threadIdx.x"], answer: 2, explain: "blockIdx.x * blockDim.x + threadIdx.x, iş parçacığının tüm ızgara içindeki benzersiz indeksidir." },
  { q: "Son blokta sınır kontrolü (if (i < N)) neden gerekli?", options: ["Performansı artırmak için", "N, blok boyutunun tam katı olmayabilir; fazladan iş parçacıkları bellek dışına yazar", "Derleyici zorunlu kılar", "Gerekli değildir"], answer: 1, explain: "Fazladan iş parçacıklarının dizi sınırı dışına yazmasını engeller." },
  ...
  { q: "Doluluğu ne belirler?", options: ["Yalnız iş parçacığı sayısı", "En kısıtlı kaynak: yazmaç, paylaşımlı bellek, iş parçacığı veya blok", "Yalnız yazmaç sayısı", "GPU sıcaklığı"], answer: 1, explain: "Doluluk, en kısıtlı kaynak tarafından belirlenir." },
    ...
    <section className="cuda-simt-surface atlas-shell" aria-label="CUDA SIMT laboratuvarı">
      ...
      {tab === "overview" && (
        <section className="panel-stack">
          <SectionHead title="Heterojen sistem: kontrol CPU’da, paralel iş GPU’da" subtitle="Ana sistem kodu kerneli başlatır; aygıt kodu çok sayıda iş parçacığı olarak çalışır." badge="Ana sistem + Aygıt" />
          <div className="flow" aria-label="CPU'dan GPU'ya yürütme akışı">
            ...
            <b aria-hidden>→</b>
            <FlowNode tone="cyan" title="GPU · Aygıt" copy="SM’ler, warp zamanlayıcıları, yürütme birimleri, GDDR/HBM" />
          </div>
      ...
      {tab === "architecture" && (
        <section className="panel-stack">
          ...
          <Lesson title="Yanlış zihinsel model" copy="Bir CUDA çekirdeği, ömrü boyunca tek bir iş parçacığına ait değildir. Warp talimatları çevrimler boyunca yürütme birimlerine gönderilir; iş parçacığı durumu yazmaçlarda korunur." />
        </section>
      )}

      {tab === "simt" && (
        <section className="panel-stack">
          <SectionHead title="SIMT: tek talimat, 32 bağımsız iş parçacığı durumu" subtitle="Her şerit farklı veriye ve yazmaç durumuna sahiptir; warp ortak talimat akışını yürütür." badge="Warp = 32 iş parçacığı" />
      ...
      {tab === "lab" && (
        <section className="panel-stack">
          <SectionHead title="Kernel Laboratuvarı: problem boyutunu ızgaraya dönüştür" subtitle="1B örnek: her iş parçacığı bir elemanı işler; sınır kontrolü son bloğu güvenli tutar." badge="i = blockIdx.x × blockDim.x + threadIdx.x" />
          <div className="lab-layout">
            <div className="lab-controls">
              <Range label="Problem boyutu N" value={n} min={1} max={4096} onChange={setN} />
              <Range label="Blok boyutu" value={blockSize} suffix=" iş parçacığı" min={32} max={1024} step={32} onChange={setBlockSize} />
              <Range label="Örnek SM sayısı" value={smCount} min={1} max={8} onChange={setSmCount} />
              <div className="formula"><code>ızgara = ceil(N / blockDim)</code><strong>ceil({n} / {blockSize}) = {blocks} blok</strong></div>
              <p className="muted">SM sayısı yalnız dağıtımı gösterir; gerçek eşzamanlılık kaynak limitlerine bağlıdır.</p>
            </div>
            <div className="lab-stage">
              <div className="stats"><Stat label="Izgara" value={`${blocks} blok`} /><Stat label="Toplam warp" value={String(totalWarps)} /><Stat label="Fazladan iş parçacığı" value={String(extraThreads)} /></div>
              <div><h3>Blokların SM’lere olası dalga dağılımı</h3><div className="sm-grid" style={{ gridTemplateColumns: `repeat(${Math.min(smCount, 4)}, minmax(0, 1fr))` }}>{Array.from({ length: smCount }, (_, sm) => { const owned = Array.from({ length: blocks }, (_, block) => block).filter((block) => block % smCount === sm); return <div className="sm-column" key={sm}><strong>SM {sm}</strong>{owned.slice(0, 8).map((block) => <span key={block} className={block === blocks - 1 ? "last" : ""}>Blok {block}</span>)}{owned.length === 0 && <small>bekliyor</small>}{owned.length > 8 && <small>+{owned.length - 8} blok</small>}</div>; })}</div></div>
          ...
          <Lesson title="Blok boyutu tek başına cevap değildir" copy="128 veya 256 iş parçacığı yararlı başlangıç deneyleridir; doğru seçim profil verisi, yazmaç ve paylaşımlı bellek kullanımı, gecikme saklama ve bellek davranışıyla ölçülür." />
        </section>
      )}
      {tab === "latency" && (
        <section className="panel-stack" role="tabpanel">
          <SectionHead title="Gecikme saklama: bellek beklerken SM boş durmaz" subtitle="Bir warp bellekten veri beklerken zamanlayıcı hazır başka bir warpa geçer; yeterli warp yoksa SM boş kalır." badge={"Periyot = " + computeCycles + " + " + latency + " = " + period + " çevrim"} />
          ...
          <Lesson title="Kritik eşik" copy={warps >= neededWarps ? "Bu basitleştirilmiş modelde yeterli warp var (" + warps + " ≥ " + neededWarps + "): zamanlayıcı bellek beklemeleri sırasında çalıştırılabilir warp bulabilir." : "Bu basitleştirilmiş modelde warp sayısı yetersiz (" + warps + " < " + neededWarps + "): tüm warplar bellek beklerken SM boş kalabilir. Modelin eşiği ceil((C+L)/C) = " + neededWarps + " warp'tır; gerçek sonuç talimat karışımına ve donanıma bağlıdır."} />
        </section>
      )}
      {tab === "occupancy" && (
        <section className="panel-stack" role="tabpanel">
          <SectionHead title="Doluluk: bir SM'ye kaç warp sığar?" subtitle="Yazmaç, paylaşımlı bellek ve iş parçacığı sınırları aynı anda geçerlidir; en kısıtlı kaynak doluluğu belirler." badge={"Doluluk = " + Math.round(occupancy * 100) + "%"} />
          <div className="occ-controls">
            <Range label="Blok boyutu" value={occBlockSize} suffix=" iş parçacığı" min={32} max={1024} step={32} onChange={setOccBlockSize} />
            <Range label="Yazmaç / iş parçacığı" value={regsPerThread} min={16} max={255} step={8} onChange={setRegsPerThread} />
            <Range label="Paylaşımlı bellek / blok" value={sharedPerBlock} suffix=" KB" min={0} max={48} step={4} onChange={setSharedPerBlock} />
          </div>
          <div className="stats">
            <Stat label="Doluluk" value={Math.round(occupancy * 100) + "%"} />
            <Stat label="Blok / SM" value={String(blocksPerSM)} />
          ...
          <Lesson title="Kritik kural" copy="Doluluk en kısıtlı kaynak tarafından belirlenir. Yazmaç veya paylaşımlı bellek kullanımını düşürerek aynı SM'de daha fazla blok barındırabilirsin." />
        </section>
      )}
      {tab === "quiz" && (
        <section className="panel-stack" role="tabpanel">
