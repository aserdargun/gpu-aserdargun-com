...
function renderLab(kind: ModuleId, locale: Locale): ReactNode | null {
  let specialist: ReactNode | null = null;
  if (kind === "visual") specialist = <VisualLab locale={locale} />;
  if (kind === "toolchain") specialist = <ToolchainLab locale={locale} />;
  if (kind === "architecture") specialist = <ArchitectureLab locale={locale} />;
  if (kind === "memory") specialist = <MemoryLab locale={locale} />;
  if (kind === "triton") specialist = <TritonLab locale={locale} />;
  if (kind === "operators") specialist = <OperatorsLab locale={locale} />;
  if (kind === "correctness") specialist = <CorrectnessLab locale={locale} />;
  if (kind === "profiling") specialist = <ProfilingLab locale={locale} />;
  if (kind === "cutlass") specialist = <CutlassLab locale={locale} />;
  if (kind === "inference") specialist = <InferenceLab locale={locale} />;
  if (kind === "multigpu") specialist = <MultiGpuLab locale={locale} />;
  if (kind === "systems") specialist = <SystemsLab locale={locale} />;
  if (specialist == null) return null;
  return <><ConceptStudio kind={kind} locale={locale} />{specialist}</>;
}
