    ...
    <section className="cuda-simt-surface atlas-shell" aria-label="CUDA SIMT laboratory">
      ...
      {tab === "latency" && (
        <section className="panel-stack" role="tabpanel">
          ...
          <div className="latency-timeline" tabIndex={0} role="region" aria-label="Warp latency timeline">
...
function KernelCode({ n, blockSize, blocks }: { n: number; blockSize: number; blocks: number }) {
  ...
  return <pre className="kernel-code" tabIndex={0} aria-label="CUDA kernel code example"><code>{segs.map((seg, i) => typeof seg === "string" ? seg : <span key={i} className="hl">{seg.hl}</span>)}</code></pre>;
}
